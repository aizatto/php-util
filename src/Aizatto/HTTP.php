<?php

namespace Aizatto;

/**
 * http://en.wikipedia.org/wiki/List_of_HTTP_status_codes
 */
class HTTP extends Enum {

  const
    // Informational
    CCONTINUE                              = 100,
    SWITCHING_PROTOCOLS                    = 101,
    PROCESSING                             = 102,

    // Success
    OK                                     = 200,
    CREATED                                = 201,
    ACCEPTED                               = 202,
    NON_AUTHORITATIVE_INFORMATION          = 203,
    NO_CONTENT                             = 204,
    RESET_CONTENT                          = 205,
    PARTIAL_CONTENT                        = 206,
    MULTI_STATUS                           = 207,
    ALREADY_REPORTED                       = 208,
    IM_USED                                = 226,
    
    // Redirection
    MULTIPLE_CHOICES                       = 300,
    MOVED_PERMANENTLY                      = 301,
    FOUND                                  = 302,
    SEE_OTHER                              = 303, 
    NOT_MODIFIED                           = 304,
    USE_PROXY                              = 305,
    TEMPORARY_REDIRECT                     = 307,
    PERMANENT_REDIRECT                     = 308,

    // Client Error
    BAD_REQUEST                            = 400,
    UNAUTHORIZED                           = 401,
    PAYMENT_REQUIRED                       = 402,
    FORBIDDEN                              = 403,
    NOT_FOUND                              = 404,
    METHOD_NOT_ALLOWED                     = 405,
    NOT_ACCEPTABLE                         = 406,
    PROXY_AUTHENTICATION_REQUIRED          = 407,
    REQUEST_TIMEOUT                        = 408,
    CONFLICT                               = 409,
    GONE                                   = 410,
    LENGTH_REQUIRED                        = 411,
    PRECONDITION_FAILED                    = 412,
    REQUEST_ENTITY_TOO_LARGE               = 413,
    REQUEST_URI_TOO_LONG                   = 414,
    UNSUPPORTED_MEDIA_TYPE                 = 415,
    REQUESTED_RANGE_NOT_SATISFIABLE        = 416,
    EXPECTATION_FAILED                     = 417,
    UNPROCESSABLE_ENTITY                   = 422,
    LOCKED                                 = 423,
    FAILED_DEPENDANCY                      = 424,
    UNORDERED_COLLECTION                   = 425,
    UPGRADE_REQUIRED                       = 426,
    PRECONDITION_REQUIRED                  = 428,
    TOO_MANY_REQUESTS                      = 429,
    REQUEST_HEADER_FIELDS_TOO_LARGE        = 431,

    // Server Error
    INTERNAL_SERVER_ERROR                  = 500,
    NOT_IMPLEMENTED                        = 501,
    BAD_GATEWAY                            = 502,
    SERVICE_UNAVAILABLE                    = 503,
    GATEWAY_TIMEOUT                        = 504,
    HTTP_VERSION_NOT_SUPPORTED             = 505,
    VARIANT_ALSO_NEGOTIATES                = 506,
    INSUFFICIENT_STORAGE                   = 507,
    LOOP_DETECTED                          = 508,
    NOT_EXTENDED                           = 510,
    NETWORK_AUTHENTICATION_REQUIRED        = 511;

  public static function isInformational($status) {
    $status = (int) $status;
    return $status >= self::CCONTINUE && $status < self::OK;
  }

  public static function isSuccess($status) {
    $status = (int) $status;
    return $status >= self::OK && $status < self::MULTIPLE_CHOICES;
  }

  public static function isRedirect($status) {
    $status = (int) $status;
    return $status >= self::MULTIPLE_CHOICES && $status < self::BAD_REQUEST;
  }

  public static function isError($status) {
    $status = (int) $status;
    return $status >= self::BAD_REQUEST;
  }

  public static function isClientError($status) {
    $status = (int) $status;
    return $status >= self::BAD_REQUEST && $status < self::INTERNAL_SERVER_ERROR;
  }

  public static function isServerError($status) {
    $status = (int) $status;
    return $status >= self::INTERNAL_SERVER_ERROR;
  }

}
